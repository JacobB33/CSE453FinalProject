...
def create_lr_scheduler(optimizer, config: LRSchedulerConfig, warmup_config: WarmUpConfig = None):
    if config.type == "step":
        ...
        lr_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, factor=config.gamma,
                                                                  patience=config.patience, mode=config.mode,
                                                                  threshold=config.threshold, min_lr=config.min_lr,
    ...
    if warmup_config is None:
        ...
        warmup_lr = torch.optim.lr_scheduler.ConstantLR(optimizer, factor=warmup_config.warmup_lr,
                                                        total_iters=warmup_config.warmup_steps)

        return torch.optim.lr_scheduler.SequentialLR(optimizer, schedulers=[warmup_lr, lr_scheduler],
